...
def songs(app, session, db):
    ...
    @app.route('/api/songs/fuzzy/<search>', methods=['GET'])
    def fuzzy_song(search):
        if 'username' in session and (session['permissions'] & 1):
            ...
            clean_search = db.converter.escape(search)
            cur = db.cursor()
            query = """SELECT * FROM songs WHERE (SOUNDEX(artist) LIKE
                       CONCAT('%', TRIM(TRAILING '0' FROM
                       SOUNDEX('{0}')), '%')) OR
                       (SOUNDEX(title) LIKE CONCAT('%', TRIM(TRAILING '0'
                       FROM SOUNDEX('{0}')), '%')) OR
                       (SOUNDEX(album_name) LIKE CONCAT('%',
                       TRIM(TRAILING '0' FROM SOUNDEX('{0}')), '%'))
                       ORDER BY title ASC LIMIT 0,50;""".format(clean_search)
            cur.execute(query)
            result = cur.fetchall()
            response = []
    ...
    # TODO separate log management panel for admins
