...
def ponies(app, session, db):
    ...
    def add_user():
        if 'username' in session and (session['permissions'] & 4) and (session['permissions'] & 1):
            ...
            cur.execute('''INSERT INTO ponies 
                            (username, password, permission_level) VALUES (\'{}\', \'{}\', {});'''.format(*values))
            new_user_id = cur.lastrowid
            db.commit()

            audit_log('Added a new user {} with permissions {}'.format(username, permissions), session, request)
            return {'result': '200', 'message': 'User added successfully', 'id': new_user_id}, 200
    ...
    def delete_user():
        if 'username' in session:
            ...
            if (session['permissions'] & 4) or session['username'] == username:
                ...
                return {'result': '200', 'message': 'User deleted successfully'}
        return {'result': '401', 'message': 'Authentication failed'}, 401
    ...
    def modify_user():
        if 'username' in session and (session['permissions'] & 4) and (session['permissions'] & 1):
            ...
            cur.execute("UPDATE ponies SET permission_level = {} WHERE lower(username) = '{}';".format(permissions,
                                                                                                       username_low))
            db.commit()
            audit_log('Modified user {}, changed permissions from {} to {}'.format(username, old, permissions),
                      session, request)
